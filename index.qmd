...
print(adf)
```
Based on the ADF test, all variables except Net overseas migration has p-values > 0.05, 
thus fail to reject null hypothesis of unit root.
```{r}
# Create an empty dataframe to store ADF test results for differenced variables
diff_adf <- data.frame(Dickey_Fuller = numeric(length(dataframes)), 
                       p_value =numeric(length(dataframes)))

# Set row names to variable names
rownames(diff_adf) <- names(dataframes)

# Perform ADF test for each variable after differencing
for (i in 1:length(dataframes)) {
  df <- get(names(dataframes)[i])  # Get the dataframe corresponding to the current variable
  diff_data <- diff(df$Value)      # Take the first difference of the variable
  adf_tmp <- adf.test(diff_data)   # Perform ADF test on the differenced series
  diff_adf[i, "Dickey_Fuller"] <- as.numeric(adf_tmp$statistic)
  diff_adf[i, "p_value"] <- as.numeric(adf_tmp$p.value)
}

# Print the ADF test results for differenced variables
print(diff_adf)
```
